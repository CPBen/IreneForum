#!/usr/bin/perl
#------------------------------------------------------------------------------
#    IreneForum - Web-based discussion forum
...
# Init
my ($m, $cfg, $lng, $user, $userId) = Irene::IreneMain->new($_[0]);

# Check if OpenID is enabled
$cfg->{openId} or $m->error('errNoAccess');

# Load additional modules
require Irene::Security::Captcha if $cfg->{captchaOpenId};
...
if ($submitted || $openIdMode) {
	...
	if ($submitted && !$openIdUrl) {
		...
		# Check captcha
		Irene::Security::Captcha::checkCaptcha($m, 'regCpt') if $cfg->{captchaOpenId};
		...
		if ($csr->user_cancel()) {
			...
			my $nonceValid = 0;
			$cache->get_and_set("irene.nonce:$nonce", sub { $nonceValid = $_[1]; undef });
...
if (!$submitted || @{$m->{formErrors}}) {
	...
	print
		...
		$cfg->{captchaOpenId} ? Irene::Security::Captcha::captchaInputs($m, 'regCpt') : "",
		$m->stdFormFields(),
		"</div>\n",
		"</div>\n",
		"</form>\n\n";
...
#------------------------------------------------------------------------------
# Redirect outside of IreneForum
...
sub redirectRaw
{
	...
	if ($Irene::IreneMain::MP) {
		$ap->status($status);
		$ap->headers_out->{'Location'} = $location;
		$ap->send_http_header() if $Irene::IreneMain::MP1;
	}
	else {
