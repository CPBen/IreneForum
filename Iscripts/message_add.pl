...
# Init
my ($m, $cfg, $lng, $user, $userId) = Irene::IreneMain->new($_[0], autocomplete => 1);
...
require Irene::Security::Captcha if $cfg->{captcha} >= 3;
...
if ($add) {
	...
	Irene::Security::Captcha::checkCaptcha($m, 'msgCpt') if $cfg->{captcha} >= 3;
...
if (!$add || @{$m->{formErrors}}) {
	...
	print
		...
		$cfg->{captcha} >= 3 ? Irene::Security::Captcha::captchaInputs($m, 'msgCpt') : "",
		"</fieldset>\n",
