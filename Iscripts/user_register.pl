...
require Irene::Security::Captcha if $cfg->{captcha};
...
if ($submitted) {
	...
	# Check captcha
	Irene::Security::Captcha::checkCaptcha($m, 'regCpt') if $cfg->{captcha};
...
if (!$submitted || @{$m->{formErrors}}) {
	...
	print
		$cfg->{captcha} ? Irene::Security::Captcha::captchaInputs($m, 'regCpt') : "",
