...
# Init
my ($m, $cfg, $lng) = Irene::IreneMain->newShell(forumId => $forumId, spawned => $spawned, upgrade => 1);
my $dbh = $m->{dbh};
my $pfx = $cfg->{dbPrefix};
my $output = "";
$| = 1;
output("IreneForum upgrade running...\n");
...
output("IreneForum upgrade done.\n");
