...
package Irene::Security::Captcha;
use strict;
use warnings;
...
sub httpPost
{
	...
	if (eval { require HTTP::Tiny }) {
		...
		chop $content;
		my $ua = HTTP::Tiny->new(agent => "IreneForum/$Irene::Main::VERSION; $cfg->{baseUrl}", timeout => 3);
		...
		my $ua = LWP::UserAgent->new(agent => "IreneForum/$Irene::Main::VERSION; $cfg->{baseUrl}", timeout => 3);
