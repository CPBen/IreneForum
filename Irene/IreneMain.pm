#------------------------------------------------------------------------------
#    IreneForum - New forum forked from IreneForum
...
#------------------------------------------------------------------------------
# Create Irene::IreneMain object for CGI/mod_perl requests
...
sub new
{
	...
	eval { require Irene::ConfigGlobal } or die "Irene::ConfigGlobal module not available"
		. " (maybe you forgot to rename Irene::ConfigGlobalDefault).";
	my $gcfg = $Irene::ConfigGlobal::gcfg;
	...
	my $m = { 
		...
		lngModule => '',    # Name of negotiated language module (e.g. "German")
	...
	if ($Irene::ConfigGlobal::gcfg->{pageTime}) {
		require Time::HiRes;
...
#------------------------------------------------------------------------------
# Create Irene::IreneMain object for commandline scripts
...
sub newShell
{
	...
	my $spawned = $params{spawned};  # Signal spawned by IreneForum, in case $MP/$CGI get inherited
...
sub initRequestObject
{
	...
	if ($MP1) {
		...
		# Use Irene::IreneCGI object
		require Irene::IreneCGI;
		Irene::IreneCGI::_reset_globals() if $FCGI;
		Irene::IreneCGI::max_read_size($cfg->{maxAttachLen});
		$m->{cgi} = Irene::IreneCGI->new();
...
sub initConfiguration
{
	...
	my $module = $hostModule || $pathModule || "Irene::Config";
	...
	my $cfg = $m->{cfg};
	if (!$cfg->{lastUpdate}) {
		require Irene::Defaults;
		$cfg->{$_->{name}} = $_->{default} for @$Irene::Defaults::options;
	}
}
...
sub setLanguage
{
	...
	return $m->{lng} if $m->loadLanguage("Irene::Lang::English");
	return {};
}
...
#	$module =~ /^Mwf[A-Za-z_0-9]+\z/ or return 0;
	$module =~ /^[A-Za-z_0-9]+\z/ or return 0;
...
#		if ($module !~ /^MwfPlg[A-Za-z_0-9]+\z/) {
	...
	if ($@ && ref($@) eq 'Irene::IreneMain::PluginError') {
...
sub printHeader 
{
	...
	print 
		"<link rel='stylesheet' href='$dataPath/ireneforum.css'>\n",
	...
	print "<script src='$dataPath/ireneforum.js' id='irenejs' data-params='$json'></script>\n";
...
#		if $m->{env}{script} ne 'forum_info' && $m->{env}{script} ne 'attach_show';
	print
		"<p class='cpr'>IreneForum",
		" $VERSION &#169; 2020 Chris Bennett</p>\n\n"
		if $m->{env}{script} ne 'forum_info' && $m->{env}{script} ne 'attach_show';
...
sub error 
{
	...
	if (!$lng->{errDefault}) {
		eval {
			...
			$m->{lng} = $lng = $Irene::Lang::English::lng;
		};
	}
...
#------------------------------------------------------------------------------
# Replace IreneForum-style named placeholders or collect names for PgSQL
...
sub dbDo
{
	...
	my $values = $values[0];
	my @pgPlaceholders = ();
	my $IrenePlaceholders = @values && ref($values) eq 'HASH';
	if ($IrenePlaceholders) {
...
sub fetchHash
{
	...
	if ($m->{pgsql}) {
		...
		if ($hr) {
			...
			tie my %h, 'Irene::IreneMain::PgHash', $hr;
			return \%h;
		}
		else { return undef }
	}
...
sub sendEmail
{
	...
	if ($cfg->{mailer} eq 'SMTP') {
		...
		require Irene::IreneSendmail;
		Irene::IreneSendmail::sendmail(smtp => $cfg->{smtpServer}, port => $port,
			from => $from, to => $to, subject => $subject, body => $body,
			'Content-Type' => "text/plain; charset=utf-8", 'X-mwForum-BounceAuth' => $bounceAuth,
			'X-Mailer' => "mwForum/$Irene::IreneMain::VERSION via Irene::IreneSendmail/$Irene::IreneSendmail::VERSION") 
			or $m->logError("Send email: $Irene::IreneSendmail::error");
	}
		...
		my $data = "From: $from\n" . "To: $to\n" . "Subject: $subject\n" . 
			...
			"X-Mailer: mwForum/$Irene::IreneMain::VERSION via Net::SMTP/$Net::SMTP::VERSION\n" .
			"\n" . $body;
		if ($cfg->{esmtpUser}) {
		...
		Mail::Sender->new()->MailMsg({ smtp => $cfg->{smtpServer}, port => $port,
			...
			headers => { 'X-mwForum-BounceAuth' => $bounceAuth,
				'X-Mailer' => "mwForum/$Irene::IreneMain::VERSION via Mail::Sender/$Mail::Sender::VERSION"	},
			@auth }) >= 0 
		...
		print $pipe
			...
			"MIME-Version: 1.0\n", "Content-Type: text/plain; charset=utf-8\n",
			"Content-Transfer-Encoding: quoted-printable\n", "X-mwForum-BounceAuth: $bounceAuth\n",
			"X-Mailer: mwForum/$Irene::IreneMain::VERSION via sendmail\n",
			"\n", $body;
		close $pipe;
	}
...
package Irene::IreneMain::PgHash;
...
package Irene::IreneMain::PluginError;
